...
z = dstopo.z
topograd = np.hypot(np.diff(z, axis=0, prepend=0), np.diff(z, axis=1, prepend=0))
topograd[0] = 0
topograd[:,0] = 0

s = 6

topograd = block_reduce(topograd, (s,s), np.max)
print(topograd.shape)
...
ix1, ix2 = find(xmin,lons), find(xmax,lons)
iy1, iy2 = find(ymax,lats)+1, find(ymin,lats)+1
...
flows *= topograd*9.8/1e3

print(flows.shape)

flows_mean = flows.mean(axis=0)
np.save("data/means/hydropower", flows_mean.data)

#print(flows_mean)

#cmap = LinearSegmentedColormap.from_list('cmap',['w','silver','dimgray','dodgerblue','blue'])
...
cb.set_label("Maximum theoretical power (kW)")
